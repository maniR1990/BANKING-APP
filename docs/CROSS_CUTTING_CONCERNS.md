...
- **Identity and Access Management (IAM):** Trust is successfully shifted to the NGINX Ingress, which forwards `X-User-ID`. However, sessions still use stateful HTTP-only cookies backed by Redis, rather than the proposed stateless JWT/OAuth2 flows.
